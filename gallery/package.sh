...
archive_temp="$(mktemp -d /tmp/bluemap-cobblefurnies-gallery.XXXXXX)"
...
(
  ...
  LC_ALL=C find . -type f -printf '%P\n' | LC_ALL=C sort |
    zip -q -X -9 "$archive_temp/cobblefurnies-gallery.zip" -@
)
unzip -tq "$archive_temp/cobblefurnies-gallery.zip"
cp "$archive_temp/cobblefurnies-gallery.zip" "$output_path"
